 ...
 *
 *	@author	Mr Greenstein & Ananya Kotla
...
public class HanoiTower {
	...
	public void solvePuzzle(String[] args) {
		...
		int peg3 = 2;
		int numDisks = levels;
		recursiveHanoi( numDisks, peg1, peg2, peg3);
	}
	
	/**		
	 *	This method autmatically solves the hanoi tower in the minimum number of moves
	 *  (2^n - 1). It does this recursively. 
	 *
	 *	@param numDisks	    Number of disks the user entered, decrements by one every 
	 												time it the method is called
	 *  @param begRod		the index of the to peg (0, 1, or 2)
	 *  @param endRod		the index of the to peg (0, 1, or 2)
	 *  @param helperRod	the index of the to peg (0, 1, or 2)
     *  @return			    void 
	 */

	 public void recursiveHanoi(int numDisks, int begRod, int endRod, int helperRod)
	 {
		if(numDisks == 1)
		{
			moveDisk(begRod, endRod);
			printTowers();
			return;
		}
		else
		{
			recursiveHanoi(numDisks-1, begRod, helperRod, endRod);
			moveDisk(begRod, endRod);
			printTowers();
			recursiveHanoi(numDisks-1, helperRod, endRod, begRod);
		}
	 }
